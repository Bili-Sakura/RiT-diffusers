...
def init_distributed_mode(args):
    ...
    if args.dist_on_itp:
        ...
        print('RANK and WORLD_SIZE and LOCAL_RANK in environ: {} {} {}'.format(args.rank, args.world_size, args.gpu))
    elif 'SLURM_PROCID' in os.environ:
        args.rank = int(os.environ['SLURM_PROCID'])
        args.gpu = args.rank % torch.cuda.device_count()
        print('SLURM_PROCID in environ: {}, will use gpu {}'.format(args.rank, args.gpu))
    else:
...
def save_on_master(*args, **kwargs):
    if is_main_process():
        print(f"Saving checkpoint {args[0]['epoch']} to: {args[1]}")  # args[1] is the checkpoint path
...
def save_model(args, model_without_ddp, optimizer, epoch, epoch_name=None):
    ...
    # make sure all processes have saved the checkpoint
...
def reduce_dict(input_dict, average=True):
    ...
    with torch.no_grad():
        keys = sorted(input_dict.keys())  # 关键！
        values = torch.stack(
